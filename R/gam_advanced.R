...
Block_residuals <- lapply(block_list, blockRMSE, equation = equation) %>%
    unlist()
...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    s(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr") + s(Load.7, bs = "cr") + s(Temp_s99,
        k = 10, bs = "cr"
    ) + as.factor(WeekDays) + BH + te(Temp_s99, Temp, bs = c(
        "cr",
        "cr"
    ), k = c(10, 10))

Block_forecast <- lapply(block_list, blockRMSE, equation = equation) %>%
    unlist()
...
summary(gam7)
# On a amélioré mais ajouté environ 16 degrés de liberté (colonne edf)
...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    ti(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr") + s(Load.7, bs = "cr") + ti(Temp_s99,
        k = 10, bs = "cr"
    ) + as.factor(WeekDays) + BH + ti(Temp_s99, Temp, bs = c(
        "cr",
        "cr"
    ), k = c(10, 10))
...
summary(gam7_ti)
# Phénomène d'interaction entre Temp_s99 et Temp
...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    s(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr") + s(Load.7, bs = "cr") + s(Temp_s99,
        k = 10, bs = "cr"
    ) + WeekDays + BH + te(Temp_s95_max, Temp_s99_max) + Summer_break +
    Christmas_break + te(Temp_s95_min, Temp_s99_min)

equation_by <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    s(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr", by = as.factor(WeekDays)) +
    s(Load.7, bs = "cr") + s(Temp_s99, k = 10, bs = "cr") + WeekDays + BH + te(
        Temp_s95_max,
        Temp_s99_max
    ) + Summer_break + Christmas_break + te(Temp_s95_min, Temp_s99_min)
# Différence de ce modèle avec celui du dessus: by weekdays, différence d'effet
# commun ou avec les jours de la semaine
...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    s(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr", by = as.factor(WeekDays)) +
    s(Load.7, bs = "cr") + s(Temp_s99, k = 10, bs = "cr") + as.factor(WeekDays) +
    BH + BH_before + BH_after + s(Temp_s95) + ti(Nebulosity_weighted) + ti(Wind_weighted) +
    ti(Wind_weighted, Temp) + s(Var1)
...
equation_cs <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") + s(toy, k = 30, bs = "cc") +
    s(Temp, k = 10, bs = "cr") + s(Load.1, bs = "cr", by = as.factor(WeekDays)) +
    s(Load.7, bs = "cr") + s(Temp_s99, k = 10, bs = "cr") + as.factor(WeekDays) +
    BH + BH_before + BH_after + s(Temp_s95) + ti(Nebulosity_weighted) + ti(Wind_weighted) +
    ti(Wind_weighted, Temp) + s(Var1, bs = "cs")
# equation_cs <- Load~s(as.numeric(Date),k=3, bs='cs') + s(toy,k=30, bs='cr') +
# s(Temp,k=10, bs='cr') + s(Load.1, bs='cs', by= as.factor(WeekDays))+
# s(Load.7, bs='cr') + s(Temp_s99,k=10, bs='cs') + as.factor(WeekDays) +BH +
# BH_before + BH_after +s(Temp_s95, bs='cs') +ti(Nebulosity_weighted, bs='ts')
# + ti(Wind_weighted, bs='ts')+ti(Wind_weighted, Temp, bs='ts') + s(Var1,
# bs='cs')
...
o <- order(Data0$toy)
plot(Data0$toy[o], terms[o, sel.column], type = "l", ylim = range(
    terms[o, sel.column],
    terms_cs[o, sel.column]
))
...
plot(Data0$Wind_weighted[o], terms[o, sel.column], type = "l", ylim = range(terms[
    o,
    sel.column
], terms_cs[o, sel.column]))
...
o <- order(Data0$Wind_weighted)
plot(Data0$Var1[o], terms[o, sel.column], type = "l", ylim = range(
    terms[o, sel.column],
    terms_cs[o, sel.column]
))
...
ssm
ssm$kalman_params
gam9.kalman.static <- ssm$pred_mean %>%
    tail(length(sel_b))
...
# dynamic using iterative grid search
ssm_dyn <- viking::select_Kalman_variances(ssm, X[sel_a, ], y[sel_a],
    q_list = 2^(-30:0),
    p1 = 1, ncores = 6
)
...
gam9.kalman.Dyn <- ssm_dyn$pred_mean %>%
    tail(length(sel_b))
...
ssm_em <- viking::select_Kalman_variances(ssm, X[sel_a, ], y[sel_a],
    method = "em",
    n_iter = 10^3, Q_init = diag(d), verbose = 10, mode_diag = T
)
...
gam9.kalman.Dyn.em <- ssm_em$pred_mean %>%
    tail(length(sel_b))
...
r <- range(cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn), cumsum(Data0$Load[sel_b] -
    gam9.forecast))
...
gam9.kalman.Dyn2 <- ssm_dyn2$pred_mean %>%
    tail(length(sel_b))
...
gqgam <- qgam(list(equation, equation_var),
    data = Data0[sel_a, ], discrete = TRUE,
    qu = 0.95
)
...
gqgam95 <- qgam(list(equation, equation_var),
    data = Data0[sel_a, ], discrete = TRUE,
    qu = 0.95
)
# gqgam05 <- qgam(list(equation, equation_var), data=Data0[sel_a,],
# discrete=TRUE, qu=0.05) X <- cbind(predict(gqgam05, newdata=Data0,
# type='terms'), predict(gqgam95, newdata=Data0, type='terms'))
...
ssm_q
gqgam.kalman.static <- ssm_q$pred_mean %>%
    tail(length(sel_b))
...
gqgam.kalman.Dyn <- ssm_dyn_q$pred_mean %>%
    tail(length(sel_b))
rmse(y = Data0$Load[sel_b], ychap = gqgam.kalman.Dyn)



res <- Data0$Load[sel_a] - ssm_dyn_q$pred_mean %>%
    head(length(sel_a))
...
gqgam05 <- qgam(list(equation, equation_var),
    data = Data0[sel_a, ], discrete = TRUE,
    qu = 0.5
)
